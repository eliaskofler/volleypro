...
from models.tournaments import Session, BeachTournament

BEACH_URL = "https://www.fivb.org/Vis2009/XmlRequest.asmx?Request=%3CRequests%3E%3CRequest%20Type=%22GetBeachTournamentList%22%20Fields=%22Season%20code%20EndDateMainDraw%20StartDateMainDraw%20EndDateQualification%20StartDateQualification%20startDate%20Name%20CountryCode%20City%20Gender%20Type%20OrganizerType%20WebSite%20EventLogos%22%3E%3CFilter%20%20FirstDate=%221900-01-01%22%20Statuses=%220%201%206%207%208%209%22/%3E%3C/Request%3E%3C/Requests%3E"
...
def fetch_beach_tournaments():
    resp = requests.get(BEACH_URL)
    root = etree.fromstring(resp.content)
    tournaments = []
    for item in root.xpath("//BeachTournament"):
        tournaments.append({
            'code': item.get("Code"),
            ...
            'start_date': parse_date(item.get("StartDateMainDraw")),
            'end_date': parse_date(item.get("EndDateMainDraw")),
...
def save_beach():
    ...
    session = Session()
    try:
        for t in fetch_beach_tournaments():
            obj = session.query(BeachTournament).filter_by(code=t['code']).first()
            if not obj:
                obj = BeachTournament(**t)
                session.add(obj)
            else:
                for k,v in t.items():
                    setattr(obj, k, v)
        session.commit()
        print(f"[Beach] Saved {len(tournaments)} tournaments")
    except Exception as e:
        print(f"[Beach] Error: {e}")
        session.rollback()
    finally:
        session.close()
